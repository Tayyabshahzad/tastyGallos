...
class OrderController extends Controller
{
    ...
    public function take(Request $request)
    {
        $data = '{
            ...
            "address":"Blue Area Islamad",
            "is_pickup":true,
            "user_id":2,
            "sub_total":955.0,
            "total":955.0,
            "payment_method":"pay_gate"
         }';
        $request2 = json_decode($data);
        $extraPrice = 0;
        $subTotal = 0;
        $discountAmount = 0;
        $extraAmount = 0;
        ...
        if(count($request2->products)>0){
            foreach ($request2->products as $product) {
                ...
                $specialPrice =  $productPrice->specialPrice->where('franchise_id', $request2->franchise_id)->first();
                if($specialPrice){
        ...
        $orderUpdate = Order::with('franchise')->findOrFail($order->id);
        if(count($request2->deal_products)>0){
             $dealTotal = 0;
            foreach ($request2->deal_products as $deal) {
                ...
                $orderDeal->save();
                if(count($deal->selected_extras)>0){
